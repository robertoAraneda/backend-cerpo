...
import { StatusCaseController } from './status-case.controller';
import { StatusCaseService } from '../services/status-case.service';

describe('StatusCasesController', () => {
  let controller: StatusCaseController;
  let service: StatusCaseService;

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      controllers: [StatusCaseController],
      providers: [StatusCaseService, StatusCaseRepository],
    }).compile();

    controller = module.get<StatusCaseController>(StatusCaseController);
    service = module.get<StatusCaseService>(StatusCaseService);
  });
